...
const { getSandboxEnv } = require('../services/sandboxEnv');

const router = express.Router();
router.use(requireAuth);

// GET /api/terminal/env -> { python, packages, managed, system, tools }
// 终端沙箱（term-runner 镜像）里实际可用的 Python 与预装库清单。
// 与前端的「项目 Python 环境」（▶ 运行 使用）是两套，这里给用户一个可见的真实清单。
// 缓存由 services/sandboxEnv 统一管理（管理员装/卸库后会失效重建）。
router.get('/env', async (req, res) => {
  res.json(await getSandboxEnv());
});
